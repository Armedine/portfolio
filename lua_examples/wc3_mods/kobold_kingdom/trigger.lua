...
function trg:sortcollapse()
    ...
    tablecollapse(self.condt)
    tablecollapse(self.actiont)
end


function trg:attachtrigu(unit)
    -- attach a condition for a triggering unit.
    self:regcond(function() return unit == trigu() end)
...
function trg:newdeathtrig(unit, func, _effect)
    ...
    local trig = trg:new("death", powner(unit))
...
function trg:newspelltrig(unit, func)
    ...
    local trig = trg:new("spell", powner(unit))
    trig:regaction(func)
    return trig
end
