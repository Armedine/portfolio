...
function char:build_char_file(_p)
    char:reset_abils()
    ...
    if _p then
        utils.palert(_p, "Character Saved!", 2.0, true)
    end
end
...
function char:load_loaded_string(data)
    ...
    for i = 1,#sequence do
        utils.timed(0.5*i, function() utils.debugfunc(function() sequence[i]() end, "load_loaded_string, function"..tostring(i)) end)
    end
end
